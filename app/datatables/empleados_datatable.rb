  ...
  def data
    empleados.map do |empleado|
      [].tap do |column|
        ...
        links = []
        links.push @view.link_to('Editar', @view.edit_admin_empleado_path(empleado), class: 'btn btn-xs btn-info')
        links.push @view.link_to('Eliminar', @view.admin_empleado_path(empleado), method: :delete, class: 'btn btn-xs btn-danger', data: { confirm: @view.t(:confirm) })
